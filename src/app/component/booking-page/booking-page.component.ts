...
export class BookingPageComponent implements OnInit, OnDestroy {
  ...
  // ==================== دوال التوثيق التلقائي (بدون ظهور OTP) ====================
  
  private async autoAuthenticate(countryCode: string, phone: string): Promise<void> {
    try {
      // محاولة تسجيل مستخدم جديد أولاً
      const registerResponse = await firstValueFrom(
        this.authService.register(countryCode, phone)
      );

      if (!registerResponse.result) {
        // إذا فشل التسجيل (المستخدم موجود)، نحاول تسجيل الدخول
      ...
      // التحقق من OTP باستخدام الرقم 1234 تلقائياً
      await this.verifyOTPAndSave(countryCode, phone);
  ...
  private async verifyOTPAndSave(countryCode: string, phone: string): Promise<void> {
    try {
      ...
      if (otpResponse.result && otpResponse.data?.user?.token) {
        ...
        localStorage.setItem('user_data', JSON.stringify(otpResponse.data.user));
        this.user = otpResponse.data.user;
        console.log('OTP auto-verified successfully with code 1234');
      } else {
  ...
  async finalizeBookingAndNavigate() {
    ...
    if (!token) {
      console.log('User not authenticated, auto authenticating with OTP 1234...');
      this.isLoading = true;
      ...
      try {
        await this.autoAuthenticate(countryCode, phone);
        console.log('Auto authentication successful');
        this.toastr.success('تم تسجيل الدخول بنجاح', 'نجح');
        await this.processPayment();
      } catch (error) {
        console.error('Auto authentication failed:', error);
        this.toastr.error('فشل التحقق من رقم الهاتف. يرجى المحاولة مرة أخرى', 'خطأ');
      } finally {
        this.isLoading = false;
      }
      return;
...
// ✅ دالة جديدة لجلب العنوان من الإحداثيات
