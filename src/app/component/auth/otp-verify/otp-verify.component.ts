...
export class OtpVerifyComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // We keep the form for validation purposes, but it will be hidden and auto-filled
    this.otpForm = this.fb.group({
      otp1: ['', [Validators.required, Validators.pattern(/^[0-9]$/)]],
      otp2: ['', [Validators.required, Validators.pattern(/^[0-9]$/)]],
      otp3: ['', [Validators.required, Validators.pattern(/^[0-9]$/)]],
  ...
  ngOnInit() {
    ...
    // --- START OF THE CHANGE: Auto-fill and submit OTP ---
    // Pre-fill the form fields with the value '1234'
    ...
    // Call onSubmit automatically after a very short delay (to ensure everything is ready)
    // This will send the verification request to the backend.
    setTimeout(() => {
      if (this.otpForm.valid) {
        this.onSubmit();
      }
    }, 100); // 100ms delay is usually enough and imperceptible to the user
    // --- END OF THE CHANGE ---
  ...
  // We keep these functions but they won't be used as the inputs are hidden.
  // If you want to completely remove the template as well, you can delete these, but they cause no harm.
  onInput(event: any, index: number) { /* Not needed now */ }
  onKeyDown(event: KeyboardEvent, index: number) { /* Not needed now */ }
  onPaste(event: ClipboardEvent) { /* Not needed now */ }
  isAllFieldsFilled(): boolean { return this.otpForm.valid; }
  ...
  onSubmit() {
    ...
    if (this.otpForm.valid) {
      ...
      // Get OTP from form (it will always be '1234')
      const otp = `${this.otpForm.value.otp1}${this.otpForm.value.otp2}${this.otpForm.value.otp3}${this.otpForm.value.otp4}`;
      
      console.log('Automatically verifying OTP:', otp); // Optional: for debugging
